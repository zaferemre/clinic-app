...
const getAppointments = async (req, res) => {
  try {
    ...
    const appointments = await Appointment_1.default
      .find({ clinicId })
...
const createAppointment = async (req, res) => {
  try {
    const { clinicId } = req.params;
    const { patientId, employeeEmail, start, end } = req.body;
    // 1) Verify patient
    ...
    // 2) Verify employeeEmail belongs to clinic
    const clinic = req.clinic; // set by authorizeClinicAccess
    if (
      clinic.employeeEmail !== employeeEmail &&
      !clinic.employees.find((w) => w.email === employeeEmail)
    ) {
      res
        .status(400)
        .json({ error: "employee does not belong to this clinic" });
      return;
    }
    // 3) Check credit
    if (patient.credit < 1) {
    ...
    const overlap = await Appointment_1.default
      .findOne({
        clinicId,
        employeeEmail,
    ...
    if (overlap) {
      res
        .status(409)
        .json({ error: "This timeslot is already taken for that employee." });
      return;
    }
    // 5) Deduct credit now
    patient.credit -= 1;
